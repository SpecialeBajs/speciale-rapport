...
The original use for GCN was proposed on node classification where each node has rich attributes, but for collaborative filtering it only has a node ID \cite{lightgcn,kipf2017semisupervised,NGCF_2019}.
This is something that LightGCN took advantage of and simplified NGCF by removing the activation function and the transformation matrix, which showed promising results.
...
BiTGCF adds some of the features from NGCF back to LightGCN, and in their experiment added GCF, which was a degenerate single domain method which utilized BiTGCF layer combination without using transfer learning.
In their experiments Meng Liu et. al showed that both GCF and BiTGCF showed large improvements over both LightGCN and NGCF, but BiTGCF only showed smaller improvements over GCF, which can indicate that high connectivity has larger effect on recommendation than transfer learning.
...
In general GCN for collaborative filtering can be abstracted as \cite{BiTGCF}
\begin{equation}
...
The embedding propagation for items can be obtained similarly as the embeddings with users by replacing $u$ to $i$ and vice versa.
