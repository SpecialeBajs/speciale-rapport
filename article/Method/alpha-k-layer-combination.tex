...
To see the impact of $\alpha_k$ we simply removed it.
If this would result in small changes in performance, it could indicate that it would not be worth while to pursuit the opportunity to optimize $\alpha_k$.
\begin{equation}
...
The initial idea of optimizing $\alpha_k$ is change each layers effect depending on how many connection each node has.
An example can be seen \autoref{tab:alpha-example}.
The numbers shown on the table are arbitrary and different variables needs to be tested to see how well it performs.
...
For a node with 200 connections, the third convolution might actually be harmful, as the node gets impacted too much by too many nodes, where 1 convolution is enough to make it perform well.
