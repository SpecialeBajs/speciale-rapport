...
But collaborative filtering suffers from the cold start problem which means that it does not deal well with new users that has not like anything yet.

\subsection{Matrix factorization}
The data concerning users and items can be represented as matrix where each row represents a user and each column an item.
The entries in the matrix will then be the explicit feedback given for that item by the user of there is any.
...
For each item i their corresponding entry in $q_i$ will meassure the extent in which the item posseses the feature and for each user u their corresponding entry in $q_u$ will meassure their liking to each feature.
...
The main challenge of using the matrix factorization model is finding a good algortihm for computing the mapping of each item to the factor vectors $q_i$ and $q_u$.
After the factor vectors are computed the task of estimating the rating a user will give an item is easy through the following equation $r_ui \: = \: q_i^T p_u$ where $r_ui$ is the resulting ratings matrix.
