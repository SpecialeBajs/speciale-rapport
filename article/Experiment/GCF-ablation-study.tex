...
\autoref{fig:GCF-NDCG-ablation-study} and \autoref{fig:GCF-NDCG-ablation-study-amazon-cell-sport} contains the GCF ablation study with all described methods in the following itemize.
...
On \autoref{fig:GCF-NDCG-ablation-study} it can be seen that the methods that utilize concatenation as layer combination generally performs worse than the methods that utilize summation.
...
On \autoref{fig:GCF-sum-NDCG-ablation-study} LightGCN performs better than the other GCF methods that utilize weighted summation as layer combination.
However on figure \autoref{fig:GCF-sum-NDCG-ablation-study-Amazon-Cell-Sport} the results varies less, beside for \textit{gcf-sum-only-ip} that is clearly the worst performing.
\begin{figure}[]
